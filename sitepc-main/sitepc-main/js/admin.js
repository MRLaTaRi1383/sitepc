        ...
        <div class="admin-item">
            <div>
                ...
                <span style="font-size:0.7rem;color:var(--muted);">
                    ${p.status === 'available' ? '🟢 موجود' : p.status === 'sold' ? '🔴 فروخته شده' : '⚪ ناموجود'}
                </span>
            </div>
            <div class="actions">
...
function setupAdminForm() {
    ...
    form.addEventListener('submit', function(e) {
        ...
        const data = { title, category, tags, price, image, description, discount: finalDiscount, status, colors };
...
function editProduct(id) {
    ...
    document.getElementById('productStatus').value = p.status || 'available';
...
function setupCategoryTagAdd() {
    document.getElementById('addCategoryBtn').addEventListener('click', function() {
        ...
        addProduct({ title: 'نمونه', category: name, tags: [], price: 0, description: '', image: '', discount: null, status: 'available', colors: [] });
    ...
    document.getElementById('addTagBtn').addEventListener('click', function() {
        ...
        const products = getProducts();
        if (products.length === 0) {
            addProduct({ title: 'نمونه', category: 'متفرقه', tags: [name], price: 0, description: '', image: '', discount: null, status: 'available', colors: [] });
        } else {
            const p = products[0];
            const tags = p.tags || [];
...
function deleteCategory(name) {
    if (confirm(`آیا از حذف دسته‌بندی "${name}" مطمئن هستید؟`)) {
        const products = getProducts();
        products.forEach(p => {
            if (p.category === name) deleteProduct(p.id);
        });
        renderAdminCategories();
        updateStats();
        updateCategorySelect();
